...
create table dbo.UserRef(
    ...
    FirstName varchar(35) not null 
        constraint ck_UserRef_first_name_cannot_be_blank check(FirstName <> ''), 
    LastName varchar(35) not null 
        constraint ck_UserRef_last_name_cannot_be_blank check(LastName <> ''),
    UserName varchar(35) not null 
        constraint ck_UserRef_user_name_cannot_be_blank check(UserName <> '') 
...
create table dbo.Ingredient(
    ...
    IngredientDesc varchar(100) not null 
        constraint ck_Ingredient_desc_cannot_be_blank check(IngredientDesc <> '') 
...
create table dbo.UnitOfMeasure(
    ...
    UnitOfMeasureDesc varchar(50) not null 
        constraint ck_UnitOfMeasure_desc_cannot_be_blank check(UnitOfMeasureDesc <> '') 
...
create table dbo.Recipe(
    ...
    CaloriesPerServing int not null 
        constraint ck_Recipe_CaloriesPerServing_must_be_greater_than_zero check(CaloriesPerServing > 0),
    DateDrafted datetime not null default getdate() 
        constraint ck_Recipe_date_drafted_cannot_be_after_future_date check(DateDrafted <= getdate()),
    DatePublished datetime null
        constraint ck_Recipe_date_published_cannot_be_after_future_date check(DatePublished <= getdate()),
    DateArchived datetime null
        constraint ck_Recipe_date_Archived_cannot_be_after_future_date check(DateArchived <= getdate()),
...
-- SM Allow same day changes for all dates.
    constraint ck_Recipe_date_drafted_is_earlier_than_date_published_and_date_archived 
        check (DateDrafted < DatePublished and DateDrafted < DateArchived),
    constraint ck_Recipe_date_published_is_earlier_than_date_archived 
...
create table dbo.RecipeIngredient(
    ...
    IngredientNum int not null 
        constraint ck_RecipeIngredient_ingredient_num_must_be_greater_than_zero check(IngredientNum > 0),    
...
create table dbo.Directions(
    ...
    StepNum int not null 
        constraint ck_Directons_step_num_must_be_greater_than_zero check(StepNum > 0),    
...
create table dbo.Meal(
    ...
    MealDateCreated date not null default getdate() 
        constraint ck_Meal_date_created_cannot_be_after_future_date check(MealDateCreated <= getdate()),
...
create table dbo.Course(
    ...
    CourseNum int not null
        constraint ck_Course_num_must_be_greater_than_zero check(CourseNum > 0)
...
create table dbo.CookBook(
    ...
    CookBookDateCreated date not null default getdate() 
        constraint ck_CookBook_date_created_cannot_be_after_future_date check(CookBookDateCreated <= getdate()),
    Active bit not null default 1, 
...
create table dbo.CookBookRecipe(
    ...
    RecipeNum int not null
        constraint ck_CookBookRecipe_recipe_num_must_be_greater_than_zero check(RecipeNum > 0),  
